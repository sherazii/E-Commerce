...
import OrderModel from "@/models/Coupon.model";
...
export async function PUT(request) {
  try {
    ...
    // ✅ Check existence
    const coupons = await OrderModel.find({ _id: { $in: ids } });
    if (!coupons.length) {
    ...
    await OrderModel.updateMany({ _id: { $in: ids } }, { $set: updateValue });
...
export async function DELETE(request) {
  try {
    ...
    // ✅ Check existence
    const coupons = await OrderModel.find({ _id: { $in: ids } }).lean();
    if (!coupons.length) {
      return response(false, 404, "Coupons not found");
    }
