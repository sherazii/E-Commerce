...
export async function PUT(request) {
  try {
    ...
    // ✅ Check existence
    const coupons = await ReviewModel.find({ _id: { $in: ids } });
    if (!coupons.length) {
    ...
    await ReviewModel.updateMany({ _id: { $in: ids } }, { $set: updateValue });
...
export async function DELETE(request) {
  try {
    ...
    // ✅ Check existence
    const coupons = await ReviewModel.find({ _id: { $in: ids } }).lean();
    if (!coupons.length) {
      return response(false, 404, "Coupons not found");
    }
