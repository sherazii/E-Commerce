...
const ProductDetails = ({ product, variant, colors, sizes, reviewCount }) => {
  // console.log(product, colors,variant,sizes,reviewCount);
        ...
        <div className="md:w-1/2 md:mt-0 mt-5">
          ...
          <div className="mt-5">
            ...
            <div className="flex gap-5">
              {colors.map((color) => (
                <Link href={`${WEBSITE_PRODUCT_DETAILS(product.slug)}?color=${color}&size=${variant.size}`} key={color} className={`border py-1 px-3 rounded-lg cursor-pointer hover:bg-primary hover:text-white ${color === variant.color ? 'bg-primary text-white': ''}`}>{color}</Link>
              ))}
            </div>
          </div>
        </div>
